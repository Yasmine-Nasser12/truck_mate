...
class _DriverOtpScreen4State extends State<DriverOtpScreen4>
  ...
  void _onOtpChanged(int index, String value) {
    ...
    if (enteredOtp.length == 4) _submitOtp();
  }
  ...
  void _submitOtp() async {
    ...
    _isSubmitting = true;
    // ✅ حفظ الـ email كـ driver
    final prefs = await SharedPreferences.getInstance();
    final registered = prefs.getStringList('registeredEmails') ?? [];
    final emailKey = '${widget.email.toLowerCase()}:driver';
    if (!registered.contains(emailKey)) registered.add(emailKey);
    await prefs.setStringList('registeredEmails', registered);
    await prefs.setString('lastRole', 'driver');
    if (!context.mounted) { _isSubmitting = false; return; }
    Navigator.push(
      context,
      MaterialPageRoute(
                      ...
                      builder: (_, __) => Transform.rotate(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                              color:
                                  const Color(0xFFFF8904).withOpacity(0.12),
                              width: 1,
                            ),
                          ),
                        ),
                    ...
                    AnimatedBuilder(
                      ...
                      builder: (_, __) => Icon(
                        ...
                        shadows: [
                          Shadow(
                            color: const Color(0xFF00D5BE)
                                .withOpacity(0.3 + 0.25 * _pulseAnim.value),
                            blurRadius: 20,
                          ),
                        ],
                      ),
                    ...
                    _stagger(
                      ...
                      Column(
                        children: [
                          ...
                          GestureDetector(
                            onTap: () async {
                              final resent = await Navigator.push<bool>(
                                context,
                                MaterialPageRoute(
                                  builder: (_) =>
                                      DriverOTPScreen5(email: widget.email),
                                ),
                              );
                              if (resent == true && context.mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                      content:
...
// ── Animated OTP box with spring entrance ──
