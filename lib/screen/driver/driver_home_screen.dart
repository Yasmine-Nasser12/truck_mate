/// ════════════════════════════════════════════════════════════
//  driver_home_screen.dart  — Fixed Navigation + Inline Tabs
//
//  التغييرات:
//  1. Bottom nav الـ 5 تابات شغالة inline (مش push screens)
//     Home / Trips / Earnings / Alerts / Profile — كلهم embedded
//  2. WillPopScope: لو على Home → اعرض logout confirm
//     لو على tab تاني → ارجع للـ Home tab أولاً
//  3. Logout: dialog → SharedPrefs → pushNamedAndRemoveUntil('/login')
//  4. Profile tab يعرض DriverProfileScreen inline
//  5. حل تعارض AvailableTripsScreen — استخدام alias
// ════════════════════════════════════════════════════════════
...
// AvailableTripsScreen from available_trips_screen (الأصلية)
import '/screen/driver/available_trips_screen.dart';
// driver_trip_screens نستخدمها بـ alias عشان نحل التعارض
...
class _DriverHomeScreenState extends State<DriverHomeScreen>
  ...
  void initState() {
    ...
    _progressAnim = CurvedAnimation(
        parent: _progressCtrl, curve: const Cubic(0.22, 1, 0.36, 1));
  ...
  Future<bool> _onWillPop() async {
    ...
    // على Home → اسأل logout
    await _logout();
    return false; // الـ logout هو اللي بيعمل النافيجيشن
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_prevOnline != driver.isOnline) {
      _prevOnline = driver.isOnline;
      WidgetsBinding.instance.addPostFrameCallback((_) => _restartEntrance());
    }

    if (driver.hasActiveTrip &&
        !_progressCtrl.isAnimating && _progressCtrl.value == 0) {
        ...
        body: Stack(children: [
          ...
          SafeArea(
            child: _buildCurrentTab(
                driver, theme, user, isDark, kBg, kCard, kBorder, kText, kMuted),
          ),
        ]),
  ...
  // ══════════════════════════════════════════
  //  TAB SWITCHER — inline (بدون push)
  // ══════════════════════════════════════════
  Widget _buildCurrentTab(
    DriverProvider driver, AppTheme theme, UserProvider user, bool isDark,
        ...
        Positioned(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              color: _kAmber.withOpacity(isDark ? 0.05 : 0.03),
            ),
            child: Container(
              decoration: BoxDecoration(
                shape: BoxShape.circle,
        ...
        Positioned(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              boxShadow: [BoxShadow(
                color: _kTeal.withOpacity(isDark ? 0.05 : 0.03),
                blurRadius: 70, spreadRadius: 35,
              )],
            ),
          ),
        ),
  ...
  Widget _buildHomeTab(
    ...
    final isAssigned  = hasTrip && driver.activeTrip?.status == TripStatus.accepted;
  ...
  // ── Header ──
  Widget _buildHeader(UserProvider user, DriverProvider driver,
      bool isDark, Color kText, Color kMuted) {
    ...
    return Row(children: [
          ...
          builder: (_, __) => Container(
            ...
            decoration: BoxDecoration(
              gradient: const LinearGradient(
                  colors: [_kAmber, _kTeal, Colors.transparent]),
      ...
      AnimatedBuilder(
        ...
        builder: (_, child) =>
            Transform.rotate(angle: _rotateAnim.value * 0.2, child: child),
        child: Container(
          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
          decoration: BoxDecoration(
            gradient: LinearGradient(colors: [
              const Color(0xFFC9A063).withOpacity(0.12),
              _kTeal.withOpacity(0.08),
            ]),
            borderRadius: BorderRadius.circular(20),
            border: Border.all(color: const Color(0xFFC9A063).withOpacity(0.25)),
          ),
          child: Row(children: [
      ...
      AnimatedSwitcher(
        ...
        child: driver.isOnline ? _onlinePill(isDark) : _offlinePill(isDark, kMuted),
      ),
      const SizedBox(width: 8),
      // Avatar → opens profile inline (tab 4)
      GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            gradient: LinearGradient(colors: [
              _kAmber.withOpacity(0.2), _kTeal.withOpacity(0.15),
            ]),
          ...
          child: Text(initials,
              style: const TextStyle(
                  color: _kAmber, fontWeight: FontWeight.w700, fontSize: 13)),
        ),
      ),
    ]);
  }

  Widget _onlinePill(bool isDark) => Container(
      ...
      AnimatedBuilder(
        ...
        builder: (_, __) => Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(
              color: _kGreen.withOpacity(_pulseCtrl.value * 0.5),
  ...
  Widget _offlinePill(bool isDark, Color kMuted) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      Container(width: 7, height: 7,
          decoration: BoxDecoration(shape: BoxShape.circle, color: kMuted)),
      const SizedBox(width: 5),
      Text('OFFLINE',
          style: TextStyle(color: kMuted, fontSize: 11, fontWeight: FontWeight.w700)),
    ]),
  );
  ...
  Widget _buildStatusChip(DriverProvider driver, bool isDark, Color kMuted) {
    if (driver.hasActiveTrip &&
        driver.activeTrip?.status == TripStatus.inProgress) {
      return _statusChip(
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          AnimatedBuilder(
            animation: _pulseCtrl,
            builder: (_, __) => Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(
                  color: _kTeal.withOpacity(_pulseCtrl.value * 0.6),
                  blurRadius: 4,
                )],
              ),
            ),
          ...
          const Text('In Transit',
              style: TextStyle(color: _kTeal, fontSize: 13, fontWeight: FontWeight.w600)),
        ]),
        bg: _kTeal.withOpacity(0.12),
        border: _kTeal.withOpacity(0.3),
      );
    }
    if (driver.hasActiveTrip) {
      return _statusChip(
        ...
        child: Column(children: [
          Container(
            ...
            child: Icon(Icons.local_shipping_outlined,
                color: isDark ? Colors.white.withOpacity(0.3) : const Color(0xFFB0BEC5),
                size: 36),
          ),
          const SizedBox(height: 20),
          Text("You're Offline",
            ...
            AnimatedBuilder(animation: _pulseAnim, builder: (_, child) =>
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  shape: BoxShape.circle,
                  gradient: const LinearGradient(
                      colors: [_kGreen, Color(0xFF30B0C7)],
                      begin: Alignment.topLeft, end: Alignment.bottomRight),
                  boxShadow: [BoxShadow(
                      color: _kGreen.withOpacity(0.4), blurRadius: 16, spreadRadius: 2)],
                ),
  ...
  // ── Assigned Hero ──
  Widget _buildAssignedHero(DriverProvider driver, bool isDark,
      Color kCard, Color kBorder, Color kText, Color kMuted) {
    return Container(
      decoration: BoxDecoration(
      ...
      child: Column(children: [
        Container(
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(
                colors: [_kTeal.withOpacity(0.15), _kTeal.withOpacity(0.08)]),
            borderRadius: const BorderRadius.vertical(top: Radius.circular(24)),
            border: Border(bottom: BorderSide(color: _kTeal.withOpacity(0.15))),
          ),
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('Trip Ready to Start',
                style: TextStyle(color: _kTeal, fontSize: 15, fontWeight: FontWeight.w700)),
            Text(driver.activeTrip?.id ?? 'SHP-4521',
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            _infoRow(kText, kMuted, 'Cargo', 'Electronics – Fragile'),
            const SizedBox(height: 10),
  ...
  // ── In-Transit Hero ──
  Widget _buildInTransitHero(DriverProvider driver, bool isDark,
      Color kCard, Color kBorder, Color kText, Color kMuted) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Align(alignment: Alignment.centerRight,
            child: Text(driver.activeTrip?.id ?? 'SHP-4521',
                style: TextStyle(color: kMuted, fontSize: 13))),
        const SizedBox(height: 12),
        _routeSection(driver, isDark, kText, kMuted),
        const SizedBox(height: 20),
          ...
          Expanded(child: Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const Text('45 min', style: TextStyle(
                  color: _kAmber, fontSize: 22, fontWeight: FontWeight.w800)),
            ]),
          )),
          ...
          Expanded(child: Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const Text('65%', style: TextStyle(
                  color: _kTeal, fontSize: 22, fontWeight: FontWeight.w800)),
            ]),
          )),
        ]),
        const SizedBox(height: 16),
            ...
            builder: (_, __) => Stack(children: [
              ...
              FractionallySizedBox(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(
                        colors: [_kAmber, _kTeal, Color(0xFF00BBA7)]),
                    borderRadius: BorderRadius.circular(6),
                    boxShadow: [BoxShadow(
                        color: _kTeal.withOpacity(0.4), blurRadius: 8)],
                  ),
                  child: ClipRRect(
                    borderRadius: BorderRadius.circular(6),
                    child: AnimatedBuilder(
  ...
  Widget _buildFragileTag() => Container(
    ...
    child: const Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      SizedBox(width: 5),
      Text('FRAGILE',
          style: TextStyle(color: _kAmber, fontSize: 11, fontWeight: FontWeight.w700)),
    ]),
  );

  // ── Incoming trips ──
  Widget _buildIncomingTripsSection(bool isDark, Color kCard, Color kBorder,
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        GestureDetector(
          ...
          child: Text(_showOffer ? 'Hide Offer' : 'Show Offer',
              style: const TextStyle(color: _kTeal, fontSize: 14)),
        ),
      ]),
      const SizedBox(height: 14),
      AnimatedSwitcher(
  ...
  Widget _buildNoOffersCard(bool isDark, Color kCard, Color kBorder,
      Color kText, Color kMuted) {
    return Container(
        ...
        AnimatedBuilder(
          ...
          builder: (_, child) => Transform.rotate(
            ...
            child: Transform.scale(
                scale: 1.0 + _pulseCtrl.value * 0.05, child: child),
          ),
          child: Container(
            width: 56, height: 56,
            decoration: BoxDecoration(
                shape: BoxShape.circle, color: _kTeal.withOpacity(0.12)),
  ...
  Widget _buildOfferCard(bool isDark, Color kCard, Color kBorder,
      Color kText, Color kMuted) {
    return Container(
          ...
          builder: (_, __) => Container(
            decoration: BoxDecoration(
              ...
              gradient: RadialGradient(
                colors: [
                  _kTeal.withOpacity(0.08 * _pulseCtrl.value),
                  Colors.transparent,
                ],
              ),
            ),
            ...
            AnimatedBuilder(
              ...
              builder: (_, __) => Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(
                    color: Color.lerp(_kTeal, _kAmber, _pulseCtrl.value)!.withOpacity(0.5),
                  ),
                ),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  AnimatedBuilder(
                    ...
                    builder: (_, __) => Container(
                      ...
                      decoration: BoxDecoration(
                        shape: BoxShape.circle, color: _kTeal,
                        boxShadow: [BoxShadow(
                          color: _kTeal.withOpacity(_pulseCtrl.value * 0.5),
                          blurRadius: 4,
                        )],
                      ),
                    ),
                  ...
                  const Text('NEW TRIP REQUEST',
                      style: TextStyle(color: _kTeal, fontSize: 11, fontWeight: FontWeight.w700)),
                ]),
              ),
            ),
            const SizedBox(height: 12),
            _tripRouteRow(kText, kMuted,
                'Sheikh Zayed Distribution Center', 'Obour Industrial Area'),
            const SizedBox(height: 12),
            Row(children: [
              Text('28 km', style: TextStyle(color: kMuted, fontSize: 12)),
              Text('  •  ', style: TextStyle(color: kMuted, fontSize: 12)),
              Text('45 min', style: TextStyle(color: kMuted, fontSize: 12)),
            ]),
            const SizedBox(height: 14),
            // View Offer → فتح available trips screen بـ push
            _buildGradientBtn('View Offer', Icons.arrow_forward,
                () => Navigator.push(context,
                    MaterialPageRoute(
                        builder: (_) => const AvailableTripsBrowseScreen()))),
          ]),
        ),
      ]),
    );
    ...
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Column(children: [
        Container(width: 8, height: 8,
            decoration: const BoxDecoration(shape: BoxShape.circle, color: _kTeal)),
      ...
      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
        Text('Payout', style: TextStyle(color: kMuted, fontSize: 11)),
        const Text('520 EGP',
            style: TextStyle(color: _kAmber, fontSize: 16, fontWeight: FontWeight.w800)),
      ]),
    ]);
  }

  // ── Today Stats ──
  Widget _buildTodayStats(DriverProvider driver, bool isDark,
      Color kCard, Color kBorder, Color kText, Color kMuted) {
    final stats = driver.todayStats;
    final items = [
      {'icon': Icons.trending_up, 'color': _kAmber,
        'label': 'Trips completed', 'value': '${stats.tripsCompleted}'},
      {'icon': Icons.attach_money, 'color': _kTeal,
        'label': 'Earnings', 'value': '${stats.earnings.toStringAsFixed(1)} EGP'},
      {'icon': Icons.access_time, 'color': const Color(0xFFFBBF24),
        'label': 'Online time', 'value': stats.onlineTime},
    ];
      ...
      ...items.asMap().entries.map((e) {
        ...
        return _animItem(
          ...
          Padding(
            ...
            child: _buildStatRow(
              isDark, kCard, kBorder, kText, kMuted,
              icon: item['icon'] as IconData,
              iconColor: item['color'] as Color,
              label: item['label'] as String,
              value: item['value'] as String,
              // Earnings row → يفتح tab الـ Earnings inline
              onTap: i == 1 ? () => setState(() => _navIndex = 2) : null,
            ),
          ),
        );
      }),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          if (onTap != null) ...[
            const SizedBox(width: 4),
            Icon(Icons.chevron_right, color: kMuted, size: 16),
          ],
        ]),
      ),
    );
  ...
  Widget _buildRecentTripsHeader(bool isDark, Color kText) =>
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text('Recent Trips',
            style: TextStyle(color: kText, fontSize: 18, fontWeight: FontWeight.w700)),
        GestureDetector(
          // View all → فتح Trips tab inline
          onTap: () => setState(() => _navIndex = 1),
          child: const Text('View all',
              style: TextStyle(color: _kTeal, fontSize: 14, fontWeight: FontWeight.w500)),
        ),
      ]);

  Widget _buildRecentTrips(DriverProvider driver, bool isDark,
      Color kCard, Color kBorder, Color kText, Color kMuted) {
    ...
    return Column(
      children: trips.asMap().entries.map((e) {
        ...
        return Padding(
          ...
          child: _animItem(7 + i,
            _TripCard(
              id: trip.id, date: trip.date,
              route: '${trip.origin} → ${trip.destination}',
              status: 'Completed', time: trip.time,
  ...
  Widget _buildGradientBtn(String label, IconData icon, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: const LinearGradient(
              colors: [_kTeal, Color(0xFF00BBA7)],
              begin: Alignment.centerLeft, end: Alignment.centerRight),
          borderRadius: BorderRadius.circular(14),
          boxShadow: [BoxShadow(
              color: _kTeal.withOpacity(0.35), blurRadius: 16, offset: const Offset(0, 6))],
        ),
        child: Stack(alignment: Alignment.center, children: [
          ClipRRect(
            borderRadius: BorderRadius.circular(14),
                  ...
                  child: Column(
                    ...
                    children: [
                      AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          gradient: active
                              ? const LinearGradient(colors: [_kTeal, Color(0xFF00BBA7)])
                              : null,
                          borderRadius: BorderRadius.circular(2),
                          boxShadow: active
                              ? [BoxShadow(color: _kTeal.withOpacity(0.4), blurRadius: 8)]
                              : [],
                        ),
                      ),
                      AnimatedSwitcher(
...
class _TripCard extends StatelessWidget {
  ...
  const _TripCard({
    required this.id, required this.date, required this.route,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text(id, style: TextStyle(color: kMuted, fontSize: 12)),
          Text(date, style: TextStyle(color: kMuted, fontSize: 12)),
        ]),
        const SizedBox(height: 8),
        Text(route, style: TextStyle(color: kText, fontSize: 14, fontWeight: FontWeight.w600)),
        const SizedBox(height: 6),
        Row(children: [
          Container(
            ...
            child: Text(status,
                style: const TextStyle(color: _kTeal, fontSize: 11, fontWeight: FontWeight.w600)),
          ),
          const SizedBox(width: 8),
