...
class TraderDummyData {
  ...
  static List<TraderNotification> notifications() {
    ...
    return List<TraderNotification>.generate(12, (index) {
      return TraderNotification(
        id: 'NOT-${3000 + index}',
  ...
  static TraderSummary summary(List<Shipment> shipments, List<DriverOffer> offers) {
    return TraderSummary(
      totalSpent: shipments.fold<double>(0, (sum, item) => sum + item.price),
      activeShipments: shipments.where((item) => item.isActive).length,
      pendingOffers: offers.where((item) => item.status == OfferStatus.pending).length,
      completedDeliveries: shipments.where((item) => item.status == ShipmentStatus.delivered).length,
    );
  }
}